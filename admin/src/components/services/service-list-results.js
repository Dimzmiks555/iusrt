...
export const ServiceListResults = ({ services, ...rest }) => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedCustomerIds = services?.rows?.map((customer) => customer.id);
    } else {
            ...
            <TableHead>
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedCustomerIds.length === services?.rows?.length}
                    color="primary"
                    indeterminate={
                  ...
                  <TableCell>
                    ...
                    >
                      <Typography
                        ...
                        variant="body1"
                      >
                        <Link href={`/services/${customer.id}`}><a>Пакет услуг № {customer.id}</a></Link>
                      </Typography>
                    </Box>
                  </TableCell>
                  <TableCell>
